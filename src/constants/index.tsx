export const EndPoints = {
  // Auth
  Login: '/api/auth/login',
  Register: '/api/auth/register',

  // Books
  Book: '/api/books',
  BooksDetail: (id: number | string) => `/api/books/${id}`,
  BooksRecommend: '/api/books/recommend',

  // Authors
  Authors: '/api/authors',
  AuthorsPopular: '/api/authors/popular',
  AuthorBooks: (id: number | string) => `/api/authors/${id}/books`,

  // Categories
  Categories: '/api/categories',

  // Cart
  Cart: '/api/cart',
  CartItems: '/api/cart/items',
  Cart_Item: (itemId: number | string) => `/api/cart/items/${itemId}`,
  CartCheckout: '/api/cart/checkout',

  // Loans
  Loans: '/api/loans',
  LoansFromCart: '/api/loans/from-cart',
  LoansMy: '/api/loans/my',
  LoansReturn: (id: number | string) => `/api/loans/${id}/return`,

  // Me
  Me: '/api/me',
  MeLoans: '/api/me/loans',
  MeReviews: '/api/me/reviews',

  // Reviews
  Reviews: '/api/reviews',
  ReviewsBook: (bookId: number | string) => `/api/reviews/book/${bookId}`,
  Review: (id: number | string) => `/api/reviews/${id}`,

  // Admin
  AdminOverview: '/api/admin/overview',
  AdminBooks: '/api/admin/books',
  AdminLoans: '/api/admin/loans',
  Admin_Loan: (id: number | string) => `/api/admin/loans/${id}`,
  AdminLoansOverdue: '/api/admin/loans/overdue',
  AdminUsers: '/api/admin/users',
};

export const Query_Keys = {
  Books: 'books',
  BooksDetail: 'bookDetail',
  BooksRecommend: 'booksRecommend',
  Authors: 'authors',
  AuthorsPopular: 'authorsPopular',
  AuthorsBook: 'authorBooks',
  Categories: 'categories',
  Cart: 'cart',
  CartCheckout: 'cartCheckout',
  Loans: 'loans',
  LoansMy: 'loansmy',
  Reviews: 'reviews',
  Me: 'me',
  MeLoans: 'meLoans',
  MeReviews: 'meReviews',
  ReviewsBook: 'reviewsBook',
  AdminOverview: 'adminOverview',
  AdminBooks: 'adminBooks',
  AdminLoans: 'adminLoans',
  AdminLoansOverdue: 'adminLoansOverdue',
  AdminUsers: 'adminUsers',
};


export const ROUTES = {
  // Auth
  Login: '/login',
  AdminLogin: '/login/admin',
  Register: '/register',

  // User
  Home: '/',
  Search: '/search',
  BookDetail: (id: number | string) => `/books/${id}`,
  Category: (id: number | string) => `/category/${id}`,
  BooksByAuthors: (id: number | string) => `/authors/${id}/books`,
  Cart: '/cart',
  CheckOut: '/checkout',
  Profile: '/profile',
  ProfileBorrowed: '/profile?tab=borrowed',
  ProfileReviews: '/profile?tab=reviews',
  BorrowSuccess: '/borrow-success',

  // Admin
  AdminRoot: '/admin',
  AdminDashboard: '/admin/dashboard',
  AdminBorrowed: '/admin/borrowed',
  AdminUsers: '/admin/users',
  AdminBooks: '/admin/books',
  AdminBookAdd: '/admin/books/add',
  AdminBookEdit: (id: number | string) => `/admin/books/${id}/edit`,
  AdminBookPreview: (id: number | string) => `/admin/books/${id}`,
  
  // Other
  NotFound: '*',
};
